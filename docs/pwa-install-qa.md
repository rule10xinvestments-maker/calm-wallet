# Calm Wallet PWA Install QA

Calm Wallet exposes install metadata through the Next.js App Router manifest at `/manifest.webmanifest`.
...
- Confirm the app name is `Calm Wallet`, manifest `id` is `/`, `start_url` is `/assistant`, display mode is `standalone`, and install icons are present.
...
- In Chrome for Android, clear Calm Wallet site data and remove any installed Calm Wallet icon before re-testing install prompts so stale manifest or WebAPK metadata is not reused.
