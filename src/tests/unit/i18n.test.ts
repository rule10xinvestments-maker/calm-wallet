...
describe("i18n helper", () => {
  ...
  it("returns audited import and notification labels in supported locales", () => {
    ...
    expect(t("activity.time.dateUnavailable", "en")).toBe("Date unavailable");
    expect(t("activity.time.dateUnavailable", "ro")).toBe("Data nu este disponibil?");
    expect(t("activity.time.dateNotSet", "fr")).toBe("Date non d?finie");
    expect(t("imports.amountUnavailable", "es")).toBe("Importe no disponible");
  ...
  it("returns credit UX copy in supported locales", () => {
    ...
    expect(t("credits.unlimited.active", "en")).toBe("Unlimited active");
    expect(t("credits.unlimited.active", "ro")).toBe("Unlimited activ");
    expect(t("credits.unlimited.active", "fr")).toBe("Illimité actif");
    expect(t("credits.unlimited.active", "es")).toBe("Ilimitado activo");
    expect(t("credits.unlimited.until", "en", { date: "July 14, 2099" })).toBe("Entries are unlimited until July 14, 2099.");
    expect(t("credits.unlimited.savedCreditsRemain", "en")).toBe("Your saved credits remain available when Unlimited ends.");
    expect(t("settings.credits.unlimitedUntil", "en", { date: "July 14, 2099" })).toBe(
      "Entries are unlimited until July 14, 2099.",
    );
  ...
  it("returns Calm Insight rule copy in supported locales", () => {
    ...
    expect(t("insights.calmInsight.rules.largest_expense_category.title", "fr", { categoryLabel: "Courses" })).toBe(
      "Catégorie principale de dépenses : Courses",
    );
