...
describe("assistant composer", () => {
  ...
  it("renders the Romanian low-credit helper as one tappable two-line row without plus or dismiss controls", () => {
    ...
    expect(helper.compareDocumentPosition(screen.getByText("Mesaj"))).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
  ...
  it("opens the shared credit options sheet from the low-credit helper row", () => {
    ...
    expect(smallPackPrice).toHaveClass("text-sm", "font-semibold", "text-sky-800");
    expect(smallPackHelper.closest("p")).toHaveClass("text-xs", "font-medium", "text-slate-500");
    expect(smallPackPrice.closest("div")).toContainElement(smallPackHelper);
    expect(within(smallPack as HTMLElement).queryByText("A simple refill.")).not.toBeInTheDocument();
